...
public class NodeChecker implements INodeChecker {
    ...
    public NodeChecker(final DataManager manager,
        ...
        history = new History(manager.getDataQI().getArray().length,
                              historyMaxSize,
                              snapshotSizeDataset,
        ...
        transformer = new Transformer(manager.getDataQI().getArray(),
    ...
    public void check(final Node node) {
        ...
        if (stateMachine.getLastNode() != null) {
            history.store(stateMachine.getLastNode(),
                          currentGroupify,
                          stateMachine.getLastTransition().snapshot);
        }
        ...
        case UNOPTIMIZED:
            currentGroupify = transformer.apply(transition.projection,
                                                node.getTransformation(),
                                                currentGroupify);
            break;
        case ROLLUP:
            currentGroupify = transformer.applyRollup(transition.projection,
                                                      node.getTransformation(),
                                                      lastGroupify,
                                                      currentGroupify);
            break;
        case SNAPSHOT:
            currentGroupify = transformer.applySnapshot(transition.projection,
                                                        node.getTransformation(),
                                                        currentGroupify,
                                                        transition.snapshot);
            break;
        ...
        // ---------
        // node.groupCount = currentGroupify.size();
        // HashGroupifyEntry m = currentGroupify.getFirstEntry();
        ...
        // count++;
        // cum+=elems.get(i);
        // if (max-cum<=config.getAbsoluteMaxOutliers()){
        // break;
        // }
        // }
        // node.groupCountMustBeLowerThan = currentGroupify.size() - (count /
        // 2);
        // ---------
    ...
    public Data getBuffer() {
        return new Data(transformer.getBuffer(),
                        data.getHeader(),
                        data.getMap(),
                        data.getDictionary());
    }
    ...
    public Data transform(final Node node) {
        ...
        currentGroupify.clear();
        currentGroupify = transformer.apply(0L,
                                            node.getTransformation(),
                                            currentGroupify);
