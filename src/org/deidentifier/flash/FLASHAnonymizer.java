...
public class FLASHAnonymizer {
    ...
    class Result {
         ...
         *            the lattice
         */
        private Result(final Metric<?> metric,
                       final INodeChecker checker,
                       final Lattice lattice) {
            this.metric = metric;
     ...
     *            the metric
     */
    public FLASHAnonymizer(final int historySize,
                           final double snapshotSize,
                           final Metric<?> metric) {
        this.historySize = historySize;
        snapshotSizeDataset = snapshotSize;
        this.metric = metric;
    ...
    protected Result anonymizeInternal(final Configuration config,
        ...
        // Build the lattice
        final Lattice lattice = new LatticeBuilder(manager.getMaxLevels(),
        ...
        final FLASHStrategy strategy = new FLASHStrategy(lattice,
                                                         manager.getHierarchies());

        // Build an algorithm instance
        config.setPracticalMonotonicity(practicalMonotonicity);
        final AbstractAlgorithm algorithm = new FLASHAlgorithm(lattice,
                                                               checker,
                                                               strategy);
        // final AbstractAlgorithm algorithm = new FLASHAlgorithm2PFSR1(lattice,
        // checker,
        // strategy);
     ...
     * @throws IOException
     */
    protected FLASHResult
            anonymizeInternal(final Data data, final Configuration config) throws IOException {
        ...
        checkBeforeEncoding(config.getRelativeMaxOutliers(),
                            handle.getDefinition().getHierarchies());

        final DataManager manager = prepareDataManager(handle, config);
        final int absoulteMaxOutliers = (int) (manager.getDataQI()
                                                      .getDataLength() * config.getRelativeMaxOutliers());
     ...
     *            the manager
     */
    private void checkAfterEncoding(final Configuration config,
                                    final DataManager manager) {
        ...
        case K_ANONYMITY:
            ...
            if ((k > manager.getDataQI().getDataLength()) || (k < 1)) { throw new IllegalArgumentException("Group size k " +
                                                                                                           k +
                                                                                                           " musst be positive and less or equal than the number of rows " +
                                                                                                           manager.getDataQI()
                                                                                                                  .getDataLength()); }
            break;
        ...
        for (final GeneralizationHierarchy hierarchy : manager.getHierarchies()) {
            if (!hierarchy.isMonotonic()) { throw new IllegalArgumentException("The hierarchy for the attribute '" +
                                                                               hierarchy.getName() +
                                                                               "' is not monotonic!"); }
        }
        ...
        for (int i = 0; i < hierarchyHeights.length; i++) {
            if (minLevels[i] > (hierarchyHeights[i] - 1)) { throw new IllegalArgumentException("Invalid minimum generalization for attribute '" +
                                                                                               manager.getHierarchies()[i].getName() +
                                                                                               "': " +
                                                                                               minLevels[i] +
                                                                                               " > " +
                                                                                               (hierarchyHeights[i] - 1)); }
            if (minLevels[i] < 0) { throw new IllegalArgumentException("The minimum generalization for attribute '" +
                                                                       manager.getHierarchies()[i].getName() +
                                                                       "' has to be positive!"); }
            if (maxLevels[i] > (hierarchyHeights[i] - 1)) { throw new IllegalArgumentException("Invalid maximum generalization for attribute '" +
                                                                                               manager.getHierarchies()[i].getName() +
                                                                                               "': " +
                                                                                               maxLevels[i] +
                                                                                               " > " +
                                                                                               (hierarchyHeights[i] - 1)); }
            if (maxLevels[i] < minLevels[i]) { throw new IllegalArgumentException("The minimum generalization for attribute '" +
                                                                                  manager.getHierarchies()[i].getName() +
     ...
     *            the hierarchies
     */
    private void checkBeforeEncoding(final double relativeMaxOutliers,
                                     final Map<String, String[][]> hierarchies) {

        // Perform sanity checks
        if ((relativeMaxOutliers < 0d) || (relativeMaxOutliers >= 1d)) { throw new IllegalArgumentException("Suppression rate " +
                                                                                                            relativeMaxOutliers +
                                                                                                            "must be in [0,1["); }
        if (hierarchies.size() > 15) { throw new IllegalArgumentException("The curse of dimensionality strikes. Too many quasi-identifiers: " +
                                                                          hierarchies.size()); }
     ...
     */
    public FLASHResult
            kAnonymize(final Data data, final int k, final double maxOutliers) throws IllegalArgumentException,
                                                                              IOException {

        if (data == null) { throw new NullPointerException("Data cannot be null!"); }
        final Configuration config = Configuration.getKAnonymityConfiguration(maxOutliers,
                                                                              k);
     ...
     */
    public FLASHResult
            lDiversify(final Data data,
                       final double c,
                       final int l,
                       final double maxOutliers) throws IllegalArgumentException,
                                                IOException {

        if (data == null) { throw new NullPointerException("Data cannot be null!"); }
        final Configuration config = Configuration.getLDiversityConfiguration(maxOutliers,
                                                                              c,
                                                                              l);
    ...
    public FLASHResult
        ...
        if (entropy) {
            config = Configuration.getLDiversityConfiguration(maxOutliers,
                                                              l,
                                                              FLASHConfiguration.LDiversityCriterion.ENTROPY);
        } else {
            config = Configuration.getLDiversityConfiguration(maxOutliers,
                                                              l,
                                                              FLASHConfiguration.LDiversityCriterion.DISTINCT);
        }
     ...
     */
    private DataManager
            prepareDataManager(final DataHandle handle,
        ...
        if (config.getSensitiveHierarchy() != null) {
            sensitive.put(handle.getDefinition()
                                ...
                                .iterator()
                                .next(), config.getSensitiveHierarchy()
                                               .getHierarchy());
        } else {
    ...
    public void setMaximumSnapshotSizeDataset(final double snapshotSize) {
        // Perform sanity checks
        if ((snapshotSize <= 0d) || (snapshotSize > 1d)) { throw new IllegalArgumentException("Snapshot size " +
                                                                                              snapshotSize +
                                                                                              "must be in [0,1]"); }
        snapshotSizeDataset = snapshotSize;
    }
     ...
     */
    public void
            setMaximumSnapshotSizeSnapshot(final double snapshotSizeSnapshot) {
        // Perform sanity checks
        if ((snapshotSizeSnapshot <= 0d) || (snapshotSizeSnapshot > 1d)) { throw new IllegalArgumentException("Snapshot size " +
                                                                                                              snapshotSizeSnapshot +
                                                                                                              "must be in [0,1]"); }
    ...
    public FLASHResult
        ...
        if (data == null) { throw new NullPointerException("Data cannot be null!"); }
        final Configuration config = Configuration.getTClosenessConfiguration(maxOutliers,
                                                                              k,
                                                                              t);
        if (data.getDefinition().getSensitiveAttributes().size() == 0) { throw new IllegalArgumentException("You need to specify a sensitive attribute!"); }
    ...
    public FLASHResult
            tClosify(final Data data,
                     final int k,
                     final double t,
                     final double maxOutliers,
                     final Hierarchy hierarchy) throws IllegalArgumentException,
                                               IOException {
        ...
        if ((hierarchy == null) || (hierarchy.getHierarchy().length == 0) ||
            (hierarchy.getHierarchy()[0].length == 0)) { throw new IllegalArgumentException("You need to specify a generalization hierarchy for the sensitive attribute!"); }
